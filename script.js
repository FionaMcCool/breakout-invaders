...
class SpaceBreakout extends Phaser.Scene {
  ...
  create() {
    ...
    this.aliens = this.physics.add.group({
      ...
      frameQuantity: 10,
      gridAlign: { width: 10, height: 3, cellWidth: 504, cellHeight: 400, x: 112, y: 50 },
    });
    ...
    this.ball = this.physics.add.image(136, 124, "assets", "ball").setCollideWorldBounds(true).setBounce(1, 1); // Ensure the ball bounces off world bounds and objects
    this.ball.setData("onPaddle", true);
    this.ball.setBounce(1, 1); // Perfectly elastic collision

    // Ensure the paddle is a physics object and immovable
    this.paddle = this.physics.add
      .image(780, 295, "assets", "paddle")
      .setImmovable(true)
  ...
  alienShoots() {
    ...
    if (aliens.length > 0) {
      ...
      if (alien) {
        // Check if 'laser' frame exists in the 'assets' atlas
        console.log("Shooting laser from", alien.x, alien.y);
        const laser = this.lasers.create(alien.x, alien.y + 16, "assets", "monsterbullet").setVelocityY(200);
        if (!laser) {
          console.error("Failed to create laser");
        } else {
          ...
          laser.body.world.on("worldbounds", (body) => {
